'use strict'; // Returns a function, that, as long as it continues to be invoked, will not
...
$(function () {
  ...
  function loadTextarea(name) {
    var id = getAutosaveID(name),
        should_restore; // Only attempt to restore if there's saved content
    // and we haven't restored it yet.

    if (localStorage.getItem(id) && !dataRestored) {
      should_restore = // Empty..
      editor.getValue().length === 0 || // ..or different that what we have saved
  ...
  var oldCodeEl = $('#code');
  var oldWrapEl = $('#wrap'); // Save original contents for reset functionality
  ...
  loadTextarea(window.location.href); // If the editor contains the default script then move cursor to the main func
